...
class JsonLoader(var tableName: String) {
    ...
    for (i <- 0 until arr.length) {
      ...
      while (keys.hasNext) {
        ...
        arr.getJSONObject(i).get(key) match {
          case t: String =>
            //the value is like 1 -> TypeInfo("name", "String")
            tableHeaderIndexMap += (Integer.valueOf(i) -> TypeInfo(key, t))
            //the value is like "name" -> "String"
  ...
  private[this] def parseTableContent(root: JSONObject): Unit = {
    ...
    for (i <- 0 until arr.length) {
      //content is  "Tuple": "{eqinson1}{1}{software engineer}{SH}{TRUE}"
      // content value is "{eqinson1}{1}{software engineer}{SH}{TRUE}"
